...
def run_minizinc(process_id, model_file, data_file, solve_time_limit=10000, 
    ...
    while process.poll() != 0:
        while True:
            line = process.stdout.readline().decode('gbk')
            if not line:
                break
            stdout += line
        ...
        sys.stdout.flush()
        while True:
            line = process.stderr.readline().decode('gbk')
            if not line:
                break
            stderr += line
...
def submit_solution(assignment_key, email_address, token, results):
    ...
    try:
        res = urlopen(req, json.dumps(submission).encode('gbk'))
    except HTTPError as e:
        responce = json.loads(e.read().decode('gbk'))
    ...
    code = res.code
    responce = json.loads(res.read().decode('gbk'))
