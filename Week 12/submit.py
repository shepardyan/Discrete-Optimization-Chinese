...
minizinc_cmd = 'mzn-cbc'

Metadata = namedtuple('Metadata', ['assignment_key', 'name', 'problem_data', 'model_data', 'submit_url'])
...
def load_metadata(metadata_file_name='_mooc'):
    '''
    ...
    try:
        with open(metadata_file_name, 'r') as metadata_file:
            metadata_object = json.load(metadata_file)
            key = metadata_object["assignmentKey"]
            name = metadata_object["name"]
            submit_url = metadata_object["submissionURL"]
            problem_data = []
            for i in metadata_object["solutionAssignments"]:
                problem_data.append(Problem(i["id"],i["model"],i["data"],i["timeout"],i["name"]))
            model_data = []
            for i in metadata_object["modelAssignments"]:
                model_data.append(Model(i["id"],i["model"],i["name"]))
            metadata_file.close()
        ...
        print(e)
        quit()
    return Metadata(key, name, problem_data, model_data, submit_url)
...
def run_minizinc(process_id, model_file, data_file, solve_time_limit=10000, 
    ...
    '''
    Executes MiniZinc and CBC on a model and data file.
    ...
    cmd = [minizinc_cmd, '-Gstd', model_file, data_file]
    if all_solutions:
        cmd.append('-a')
    if isinstance(solve_time_limit, int):
        cmd.append('--timeout')
        cmd.append(str(solve_time_limit))

    print('running cbc minizinc as a subprocess with the command,')
    print(' '.join(cmd))
...
def output(problem, model_file=None):
    ...
    try:
        stdout, stderr = run_minizinc(
            ...
            model_file,
            problem.input_file,
            solve_time_limit=int(problem.runtime),
...
def login_dialog(submit_url, assignment_key, results, credentials_file_location = '_credentials'):
    '''
    ...
    while not success:
        ...
        code, responce = submit_solution(submit_url, assignment_key, login, token, results)
...
def submit_solution(submit_url, assignment_key, email_address, token, results):
    '''
...
def main(args):
    ...
    if args.credentials is None:
        login_dialog(metadata.submit_url, metadata.assignment_key, results)
    else:
